...
const Checkbox = ({ checked, onToggle }) => (
	<div
		className={`nsc checkbox-display ${checked ? "checked" : ""}`}
		onClick={onToggle}
	...
	>
		<svg
			...
			height="16"
			fill="currentColor"
			className="nsc input-icon bi bi-check-lg"
			viewBox="0 0 16 16"
		>
...
const Radio = ({ checked, onToggle }) => (
	<div
		className={`nsc radio-display ${checked ? "checked" : ""}`}
		onClick={onToggle}
		ischecked={checked ? "true" : "false"}
	></div>
);
...
const Input = (props) => {
	...
	useEffect(() => {
		...
		if (props.type === "button") {
			setWarning(
				"WARNING: Use the included NSC Button component. (it's much better :D)"
			);
		}
	}, [props]);
						...
						>
							{visible ? (
								<svg
									...
									height="16"
									fill="currentColor"
									className="nsc input-icon bi bi-eye"
									viewBox="0 0 16 16"
								>
								...
								<svg
									...
									height="16"
									fill="currentColor"
									className="nsc input-icon bi bi-eye-slash"
									viewBox="0 0 16 16"
								>
	...
	return (
		<div
			...
			className={`nsc input${className ? " " + className : ""}`}
		>
			{error && <div className="nsc error">{error}</div>}
			{warning && <div className="nsc warning">{warning}</div>}
